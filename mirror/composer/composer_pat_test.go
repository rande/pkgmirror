...
func Test_Composer_Pat_AllVariables(t *testing.T) {
	p := NewPackagePat("packagist")
	...
	assert.Equal(t, "kevinlebrun", vars["vendor"])
	assert.Equal(t, "colors.php", vars["package"])
	assert.Equal(t, "f8ef02dddbd0bb7f78a2775e7188415e128d7b147f2a5630784c75cfc46a1a7e", vars["ref"])
	assert.Equal(t, "json", vars["format"])
}

func Test_Composer_Pat_OtherVariable(t *testing.T) {
	p := NewPackagePat("packagist")
